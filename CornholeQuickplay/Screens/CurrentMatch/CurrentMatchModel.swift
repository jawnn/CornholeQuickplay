...
class CurrentMatchModel: CurrentMatchModelType {
    ...
    init(match: Match) {
        ...
        self.currentFrame = Frame(frame: self.currentMatch.currentFrameNumber, bluePitcher: FrameStat(pitcher: self.bluePitcher), redPitcher: FrameStat(pitcher: self.redPitcher))
    }
